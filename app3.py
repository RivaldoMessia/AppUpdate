...
with tab2:
    ...
    st.markdown(f"**Model Performance**: R² = {r2:.2f}, MAE = {mae:.2f} Q/acre")
...
with tab4:
    ...
    if st.button("Generate Planting Schedule", key="schedule_button"):
        ...
        if suitable_dates:
            ...
            date_range = f"{min(suitable_dates).strftime('%B %d')} - {max(suitable_dates).strftime
# ---- SESSION STATE LOGIN INIT ----
if "authenticated" not in st.session_state:
    st.session_state["authenticated"] = False
if "login_attempted" not in st.session_state:
...
def get_coordinates(city):
    geolocator = Nominatim(user_agent="AgriLens")
    try:
...
def get_weather_data(city):
    ...
    if lat == 0 and lon == 0:
        st.warning(f"Could not find coordinates for {city}. Using simulated data.")
        return get_simulated_weather(city)
...
with st.container():
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.image("https://via.placeholder.com/800x200.png?text=Crop+Field+Image", caption="Visualize Your Farming Future", use_column_width=True)
...
with st.sidebar:
    ...
    st.markdown("<h2 style='color: #388E3C;'>🌱 Planting Scheduler</h2>", unsafe_allow_html=True)
    city = st.text_input("City (e.g., Nairobi, Iowa)", "Nairobi", key="city_input", help="Enter city for weather forecast.")
    crop = st.selectbox("Select Crop", list(CROP_REQUIREMENTS.keys()), key="crop_select", help="Choose crop for planting schedule.")
