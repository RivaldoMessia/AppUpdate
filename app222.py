...
with tab4:
    ...
    if st.button("Generate Planting Schedule", key="schedule_button"):
        ...
        if suitable_dates:
            ...
            # Display weather data for transparency
            st.markdown("#### Weather Forecast")
            weather_df["Date"] = weather_df["date"].apply(lambda x: x.strftime("%Y-%m-%d"))
            weather_df["Temperature (°C)"] = weather_df["temp"].round(1)
            weather_df["Rainfall (mm)"] = weather_df["rainfall"].round(1)
            st.dataframe(weather_df[["Date", "Temperature (°C)", "Rainfall (mm)"]], use_container_width=True)
            ...
            st.warning(f"No suitable planting dates found for {crop} in the next 7 days. Try selecting 'Millet' for broader suitability.")